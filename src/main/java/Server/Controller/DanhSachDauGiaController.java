...
public class DanhSachDauGiaController {
    ...
    private Node createAuctionCard(Auction auction) {
        ...
        Label session = new Label("Phiên #" + auction.getId());
        session.getStyleClass().add("data-meta");
        Label item = new Label("Sản phẩm #" + auction.getItem_id());
        item.getStyleClass().add("data-meta");
        Label minStep = new Label("Bước giá " + formatMoney(getEffectiveMinIncrement(itemData)));
